...
export default async function BlogPage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-4 sm:px-6 py-20 text-slate-100">
      <h1 className="text-4xl font-semibold">blog</h1>
      <p className="mt-4 text-slate-300">notes on backend architecture, ai systems, and linux stuff.</p>

      <section className="mt-10 space-y-4">
        {posts.map((post) => (
          <article key={post.uid} className="rounded-xl border border-slate-800 bg-[#0d1312] p-5">
            ...
            <Link className="mt-4 inline-block font-semibold text-sky-300 hover:text-sky-200" href={`/blog/${post.uid}`}>
              Read post
            </Link>
          </article>
        ))}
